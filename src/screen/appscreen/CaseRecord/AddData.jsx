...
const AddData = ({ patient }) => {
  ...
  const allOptions = {
    ...
    level2: {
        'L1O1': [ // Options for Level 1 Option 1 (History)
            { id: 'L2O1-1', name: 'Past History' },
            { id: 'L2O1-2', name: 'Family History' },
        ],
        'L1O2': [  // Options for Level 1 Option 2 (Attributes)
            { id: 'L2O2-1', name: 'Mental' },
            { id: 'L2O2-2', name: 'Physical' },
        ],
        'L1O3': [ // Options for Level 1 Option 3 (Expression) -  Keep consistent structure
        ...
        'L1O4': [ // Options for Level 1 Option 4 (Key Event) - Keep consistent
            { id: 'L2O4-1', name: 'No' },
    ...
    level4: {  // ONLY for Expression items
        ...
        'L3O3-1-2': [ // Physical (under Location)
            { id: 'L4O3-1-2-1', name: 'General' },
            { id: 'L4O3-1-2-2', name: 'Perticalur' },
        ],
        'L3O3-2-1': [ // Mental (under Sensation)
        ...
        'L3O3-2-2': [ // Physical (under Sensation)
            { id: 'L4O3-2-2-1', name: 'General' },
            { id: 'L4O3-2-2-2', name: 'Perticalur' },
        ],
        'L3O3-3-1': [ // Mental (under Pathology)
        ...
        'L3O3-3-2': [ // Physical (under Pathology)
            { id: 'L4O3-3-2-1', name: 'General' },
            { id: 'L4O3-3-2-2', name: 'Perticalur' },
        ],
        'L3O3-4-1': [ // Mental (under Sign)
    ...
    level5: {
     ...
     level6: {
        'L5O3-1-1-1-1' : [
            {id: 'L6O3-1-1-1-1-1', name: "Option L6-Mental-Loc"}
        ],
        'L5O3-1-2-1-1' : [
            {id: 'L6O3-1-2-1-1-1', name: "Option L6-Physical-Loc"}
        ],
         'L5O3-2-1-1-1' : [
            {id: 'L6O3-2-1-1-1-1', name: "Option L6-Mental-Sen"}
        ],
        'L5O3-2-2-1-1' : [
            {id: 'L6O3-2-2-1-1-1', name: "Option L6-Physical-Sen"}
        ],
        'L5O3-3-1-1-1' : [
            {id: 'L6O3-3-1-1-1-1', name: "Option L6-Mental-Path"}
        ],
        'L5O3-3-2-1-1' : [
            {id: 'L6O3-3-2-1-1-1', name: "Option L6-Physical-Path"}
        ],
        'L5O3-4-1-1-1' : [
            {id: 'L6O3-4-1-1-1-1', name: "Option L6-Mental-Sign"}
        ],
        'L5O3-4-2-1-1' : [
            {id: 'L6O3-4-2-1-1-1', name: "Option L6-Physical-Sign"}
        ],
        'L5O3-5-1-1-1' : [
            {id: 'L6O3-5-1-1-1-1', name: "Option L6-Cause-mod"}
        ],
         'L5O3-5-2-1-1' : [
  ...
  const getCurrentOptions = () => {
    if (currentLevel === 'level1') {
    ...
    } else if (currentLevel === 'level4' && parentOptionId) {  // Add level 4
    ...
    } else if (currentLevel === 'level5' && parentOptionId) { // Add level 5
      ...
      else if (currentLevel === 'level6' && parentOptionId) { // Add level 6
...
const handleOptionSelect = (option, rowIndex) => {
    setSelectedOptions(prev => {
        ...
        if (newOptions[rowIndex].includes(option.name)) {
            newOptions[rowIndex] = newOptions[rowIndex].filter(o => o !== option.name);
        } else {
...
const handleBack = () => {
    if (currentLevel === 'level6') {  // Add level 6
      setCurrentLevel('level5');
      setCurrentPage(0);
    }else if (currentLevel === 'level5') {  // Add level 5
        setCurrentLevel('level4');
        setCurrentPage(0);
    }else if (currentLevel === 'level4') { // Add level 4
        setCurrentLevel('level3');
        setCurrentPage(0);
    } else if (currentLevel === 'level3') {
        setCurrentLevel('level2');
          ...
          <div className="modal-dialog modal-dialog-scrollable" role="document">
            <div className="modal-content">
              ...
              <div className="modal-body ">
                 {selectedOptions[focusedIndex] && selectedOptions[focusedIndex].length > 0 && (
                    <div>
                ...
                {getCurrentOptions().map((option, index) => (
                      ...
                      >
                        <p className="mb-0">{option.name}</p>  {/*Display option.name*/}
                        <input
                          type="checkbox"
                          checked={selectedOptions[focusedIndex]?.includes(option.name)} // Check against option.name
                          onChange={() => handleOptionSelect(option, focusedIndex)}
                ...
                >
                 ...
                 {currentLevel === 'level3' ? (
                  ...
                  <button
                    ...
                    // onClick={handleNext}
                    disabled={
                      (currentPage + 1) * itemsPerPage >= getCurrentOptions().length
                    }
                  >
                    Next
                  </button>
